...
Model::Model(double t_length, unsigned t_discretization, std::array<double,2> t_wind,
             ...
             int local_rows, int local_offset, double t_max_wind)
    : 
      ...
      m_fire_map(t_discretization * t_discretization, 0u)
{
    ...
    // Vérifier si la position de départ du feu est dans ce sous-domaine
if (t_start_fire_position.row >= static_cast<unsigned>(m_local_offset) &&
    t_start_fire_position.row < static_cast<unsigned>(m_local_offset + m_local_rows)) {
...
bool Model::update()
{
    ...
    for (auto f : m_fire_front)
    {
        ...
        // Propagation vers le Nord (vérification des limites locales)
        if (coord.row > 0) { // Ne pas déborder en haut
        ...
        // Propagation vers le Sud (vérification des limites locales)
        if (coord.row < m_local_rows - 1) { // Ne pas déborder en bas
        ...
        // Propagation vers l'Ouest (vérification des limites locales)
        if (coord.column > 0) { // Ne pas déborder à gauche
        ...
        // Propagation vers l'Est (vérification des limites locales)
        if (coord.column < m_geometry - 1) { // Ne pas déborder à droite
...
std::size_t Model::get_index_from_lexicographic_indices(LexicoIndices t_lexico) const {
    // Convertit les indices locaux en indices globaux
    return (t_lexico.row + m_local_offset) * m_geometry + t_lexico.column;
}

Model::LexicoIndices Model::get_lexicographic_from_index(std::size_t t_global_index) const {
    LexicoIndices ind_coords;
    // Convertit les indices globaux en indices locaux
    ind_coords.row = (t_global_index / m_geometry) - m_local_offset;
    ind_coords.column = t_global_index % m_geometry;
    return ind_coords;
}
