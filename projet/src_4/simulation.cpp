...
struct TimingData {
    ...
    double ghost_comm_time = 0.0;
    double gather_comm_time = 0.0;
...
int main(int argc, char* argv[]) {
    ...
    // Définir la limite d'itérations
    const std::size_t max_iterations = 2000; // ou une valeur passée en paramètre
    ...
    if (world_rank == 0) {
        ...
        while (keep_running) {
            ...
            timing.send_time += MPI_Wtime() - start;
        ...
        std::vector<TimingData> all_timings(world_size);
        MPI_Gather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, 
                 all_timings.data(), sizeof(TimingData), MPI_BYTE, 
                 0, MPI_COMM_WORLD);

        TimingData total{};
        int max_iter = 0;
        for (const auto& t : all_timings) {
            total.compute_time += t.compute_time;
            total.ghost_comm_time += t.ghost_comm_time;
            total.gather_comm_time += t.gather_comm_time;
            total.send_time += t.send_time;
            if (t.iterations > max_iter) max_iter = t.iterations;
        }
        ...
        if (max_iter > 0) {
            std::cout << "\n=== Performance Summary ==="
                      ...
                      << "\n  Compute:       " << total.compute_time / max_iter << "s"
                      << "\n  Ghost comm:    " << total.ghost_comm_time / max_iter << "s"
                      << "\n  Gather comm:   " << total.gather_comm_time / max_iter << "s"
                      << "\n  Send/Display:  " << total.send_time / max_iter << "s"
                      << std::endl;
        }
    } 
    else {
        int num_compute_procs = world_size - 1;
        ...
        Model model(params.length, params.discretization, params.wind,
        ...
        while (simulation_running) {
            ...
            // 1. Ghost cells exchange
            double start = MPI_Wtime();
            update_ghost_cells(local_fire, local_vegetation, local_rows, cols, 
                              newComm, comp_rank, num_compute_procs);
            timing.ghost_comm_time += MPI_Wtime() - start;

            // 2. Update fire front and model
            start = MPI_Wtime();
            model.update_fire_front();
            bool local_continue = model.update();
            timing.compute_time += MPI_Wtime() - start;

            // 3. Check max iterations
            int local_max_reached = (model.time_step() >= max_iterations);
            int global_max_reached;
            MPI_Allreduce(&local_max_reached, &global_max_reached, 1, MPI_INT, MPI_MAX, newComm);

            // 4. Check global activity
            int global_active;
            MPI_Allreduce(&local_continue, &global_active, 1, MPI_INT, MPI_SUM, newComm);
            
            simulation_running = (global_active > 0) && !global_max_reached;
            ...
            for (int i = 0; i < num_compute_procs; ++i) {
                counts[i] = ((i < (total_rows % num_compute_procs)) ? 
                          (total_rows / num_compute_procs + 1) : 
                          total_rows / num_compute_procs) * cols;
                displs[i] = offset;
                offset += counts[i];
            }
            ...
            MPI_Gatherv(local_veg_data.data(), local_veg_data.size(), MPI_UINT8_T,
                      ...
                      0, newComm);
            timing.gather_comm_time += MPI_Wtime() - start;
        ...
        MPI_Gather(&timing, sizeof(TimingData), MPI_BYTE, 
